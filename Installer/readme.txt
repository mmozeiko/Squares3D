Readme for Squares 3D release for Indago 2006 game contest (version 1.1.2).

System requirements:
- Runs fairly well on systems above 1 GHz and video cards above GeForce 2.

Game rules:
- It is STRONGLY recommended to read and understand the rules of game before playing.
  Rules can be found in main menu of the game.

Controls:
- Use your mouse to move around and hit the ball with your character.
- Use space button to make your character jump.
- Press and hold the left mouse button to make combo`s (keep the ball in air increasing hits count).
- Hold down the right mouse button to rotate the camera with mouse and use mouse wheel to zoom in or out

Known issues:
- Mouse movement might have some lagging behavior if game frames per second exceeds 100.
  Solution would be to enable vertical synchronization in advanced video options. Enabling vertical
  synchronization is not recommended if you`re getting low fps.
- German and russian localizations have some text parts untranslated from english.
- Under Linux and MacOSX user must have write access rights to folder where Squares 3D executable file
  or bundle is located. You can simple keep the game in some folder on Desktop.
- Network game isn't well tested when playing on unstable connections such as internet. LAN recommended.
- Server computer should be powerful enough to render at least 50 fps. Otherwise the game for remote
  players won't feel smooth.

If game runs slow (at unplayable frame rate) it is recommended to turn down video options to lowest
detalization. Slowest video effects are usage of shadows and grass density. These two are the first
options to tweak down in case of poor performance. Also using HDR, Antialiasing and Anisotropic
filtration is not recommended on older video cards.
Applying selected options is done by pressing the apply button in options screens.

See the included licenses.txt file for information on used 3rd party library licenses.


Have fun!

Squares team.

--------------------------------------------------------------------------------------------------------------------------------------------------

Squares 3D "izlasi mani" Indago 2006 spēļu konkursam (versija 1.1.2).

Sistēmas prasības:
- Spēle iet visnotaļ labi uz sistēmām ar 1 GHz vai ātrāku procesoru un videokarti GeForce 2 vai labāku.

Spēles noteikumi:
- Pirms spēlēšanas ir stingri ieteicams izlasīt un saprast spēles noteikumus. Tie ir atrodami spēles
  galvenajā izvēlnē.

Vadība:
- Lietojiet peli, lai kustētos apkārt un sistu bumbu.
- Ar space pogu var lekt.
- Nospiediet un turiet peles kreiso pogu, lai taisītu sitienu sērijas.
- Turiet peles labo pogu, lai ar peli kustinātu speles kameru. Lietojiet peles rulli, lai pietuvinātu
  vai attālinātu kameru.
  
Zināmās problēmas:
- Peles kustība var notikt ar aizkavējumiem, ja spēles kadri sekundē (FPS) pārsniedz 100. Lai apietu
  šo problēmu uzstādiet vertikālo sinhronizāciju par ieslēgtu. Tā ir atrodama spēles papildus video
  uzstādījumos. Ja spēle iet lēni (zems FPS), tad lietot vertikālo sinhronizāciju nav ieteicams.
- Vācu un krievu lokalizācijās ir dažas daļas neiztulkotas no angļu valodas.
- Linux un MacOSX lietotājam jābūt rakstīšanas tiesībām folderim, kurā atrodas Squares 3D izpildāmais
  fails vai bundle. Spēli var vienkārši turēt uz Desktopa kādā folderī.
- Tīkla spēle nav labi notestēta spēlējot internetā. Ieteicams spēlēt LAN tīklā.
- Servera datoram ļoti ieteicams būt pietiekami jaudīgam, lai spētu attēlot vismaz 50 kadrus sekundē.
  Citādi tīkla spēlētājiem kustība būs nevienmērīga.
  
Ja spēle iet lēni (par maz kadru sekundē), tad ir ieteicams uzstādīt video opcijas uz zemākajiem
uzstādījumiem. Visvairāk resursus prasa ēnu lietošana un zāļu daudzums spēles pasaulē. Šie abi
ustādījumi ir pirmie, kurus vajadzētu pieregulēt, ja spēle neiet pietiekami ātri. Arī HDR, Antialiasing
(attēla izlīdzināšana) un anizotropiskā filtrācija nav ieteicama uz vecākām videokartēm.
Opciju apstiprināšana notiek nospiežot uzstādīšanas pogu opciju izvēlnēs.

Lai redzētu informāciju par spēlē izmantoto bibliotēku licencēm, apskatiet licences.txt failu.


Interesantu spēlēšanu!

Kvadrātu komanda.
