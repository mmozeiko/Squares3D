...
unsigned int FrameBuffer::attachColorTex(bool hdr)
{
    ...
    if (hdr)
    {
        ...
        */
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_size, m_size, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
...
unsigned int FrameBuffer::attachDepthTex(bool hdr)
{
    ...
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24_ARB, m_size, m_size, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, NULL);
    }
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
